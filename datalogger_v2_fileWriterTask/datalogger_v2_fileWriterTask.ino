...
//改成使用flush(每隔一段時間強制把數據寫入 SD 卡晶片)
void fileWriterTask(void *pvParameters) {
  ...
  // 定義自動存檔(Flush)的時間間隔
  // 設定 500ms 代表斷電時最多只損失 0.5 秒的數據
  // 不要設太短(如 10ms)，否則 SD 卡會來不及寫入導致掉包
  ...
  while (1) {
    ...
    // 檢查檔案是否還活著，如果沒開成功或斷掉，嘗試重開
    if (!logFile) {
        if (xSemaphoreTake(sdMutex, pdMS_TO_TICKS(100)) == pdTRUE) {
            logFile = SD.open(currentFilename, FILE_APPEND);
            xSemaphoreGive(sdMutex);
        }
        // 如果還是開不起來，稍等一下再試，避免死迴圈卡死 CPU
        if (!logFile) {
    ...
    while (xQueueReceive(canQueue, &msg, 0) == pdTRUE) {
      ...
      // 如果 Buffer 快滿了，先寫入 SD 卡 (但還不一定 Flush)
      if (bufferPos + lineLen >= sizeof(fileBuffer)) {
        if (xSemaphoreTake(sdMutex, pdMS_TO_TICKS(100)) == pdTRUE) {
           // 這裡只用 write，動作很快，只是把資料從 RAM 搬到 SD 卡控制器的 Buffer
    ...
    if (now - lastFlushTime > kFlushIntervalMs) {
      if (xSemaphoreTake(sdMutex, pdMS_TO_TICKS(100)) == pdTRUE) {
        ...
        // 2. 【核心動作】強制 SD 卡存檔！
        // 這行執行完，資料就真正安全了，這時候斷電也不怕
        logFile.flush(); 
        
        lastFlushTime = now;
        messagesLogged++; // 這裡計數可能不精確，但沒關係
        xSemaphoreGive(sdMutex);
      }
    }
