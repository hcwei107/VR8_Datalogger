...
constexpr uint32_t kSpiFrequency = 15000000;        // Max SPI speed for SD card
...
void loop() {
  ...
  if (alerts_triggered & TWAI_ALERT_RX_FIFO_OVERRUN) {
    // 這是最嚴重的：代表 ESP32 忙到連硬體暫存器都來不及讀
    Serial.println("CRITICAL: Hardware FIFO Overrun! (CPU 太忙或中斷被卡住)");
  }
  
  if (alerts_triggered & TWAI_ALERT_RX_QUEUE_FULL) {
    // 這是最常見的：代表 rx_queue_len 設太小，或是 SD 卡寫入卡住導致接收任務變慢
...
bool initCAN() {
  ...
  twai_general_config_t g_config = {
    ...
    .tx_queue_len = 5,
    .rx_queue_len = 100,    // 修改buffer大小(5改成100)
    //.alerts_enabled = TWAI_ALERT_NONE,
    .alerts_enabled = TWAI_ALERT_RX_QUEUE_FULL | TWAI_ALERT_RX_FIFO_OVERRUN, //新增判斷是否漏接數據
    .clkout_divider = 0
  };
...
//改成使用flush(每隔一段時間強制把數據寫入 SD 卡晶片)、並增大buffer為32KB
void fileWriterTask(void *pvParameters) {
  char lineBuffer[kMaxLineLength];
  size_t bufferPos = 0;
  
  // --- 修改點 1: 定義一個新的大容量常數 (32KB) ---
  const size_t kBigBufferSize = 32768; 

  // --- 修改點 2: 改用 malloc 動態配置記憶體 ---
  // (不要用 char fileBuffer[...]，會把 Stack 撐爆)
  char *fileBuffer = (char *)malloc(kBigBufferSize);
  
  // 檢查有沒有配置成功，如果記憶體不足就停機報錯
  if (fileBuffer == NULL) {
  ...
  while (1) {
    ...
    if (!logFile) {
        ...
        if (!logFile) { vTaskDelay(pdMS_TO_TICKS(100)); continue; }
    }
    ...
    while (xQueueReceive(canQueue, &msg, 0) == pdTRUE) {
      ...
      // --- 修改點 3: 這裡原本是 sizeof(fileBuffer)，要改成 kBigBufferSize ---
      // 判斷：如果加上新的一行會不會爆出 32KB？
      if (bufferPos + lineLen >= kBigBufferSize) { 
        if (xSemaphoreTake(sdMutex, pdMS_TO_TICKS(100)) == pdTRUE) {
           logFile.write((uint8_t*)fileBuffer, bufferPos);
           bufferPos = 0;
           xSemaphoreGive(sdMutex);
        }
      }
      
      // --- 修改點 4: 這裡原本也是 sizeof(fileBuffer)，改成 kBigBufferSize ---
      // 填入 Buffer
      if (bufferPos + lineLen < kBigBufferSize) {
